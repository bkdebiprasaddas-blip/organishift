...
> Auto-read at the start of every session. This file carries the universal
> compliance rules verbatim (§A / §D / §F / §H / §J below, per RULEBOOK §I.5)
> plus a short project-specific section. The master compliance rule book is
> `RULEBOOK.md` (canonical home: `governance\RULEBOOK.md`); it and this file
> must never contradict each other. If they ever conflict, the stricter rule
> governs (RULEBOOK §0.6).
...
## Project-specific (SHORT — the verbatim rules above are the master)

### Project
- **Name:** OrganiShift — reusable event planning, scheduling and execution
  tracking system.
- **Phase:** Phase 1 MVP (CRUD, six pages). Phase-2 features (AI plan
  generation, notifications, reminders, comments, attachments, budgets,
  dependencies, real-time collaboration, Socket.IO, weighted progress,
  marketplace, template sharing, complex analytics, GraphQL) are OUT unless
  separately authorized.
- **Roles:** ADMIN / MANAGER / MEMBER — enforce at the API, not just the UI.
- **Tech:** MERN — MongoDB, Express, React (Node). Three-tier + modular service
  layer: Route → Controller → Service → Model. Folder names may be historical;
  trust verified docs over names.
- **Branch:** `v1` (solo project; direct commits to `v1` after user approval).

### Authoritative specs (frozen)
- `OrganiShift_Phase1_System_Design.docx` — product scope, architecture, data
  model, APIs, UI, roles, algorithms, testing, Definition of Done (the "what").
- `OrganiShift_AI_Agent_Execution_Spec.md` — how the agent must operate (the
  "how").
- `OrganiShift_Agent_Build_Spec.md` — the build contract: one self-contained
  task-by-task spec (T-001…T-037) with corrections (C-1…C-11), source-defect
  fixes (S-1…S-5) and derived rules. Supersedes the System Design for **build**
  detail where it adds + resolves ambiguities; it never redefines the frozen
  source of truth.
...
### Structure (canonical)
- **ROOT:** `server/` (Express+Mongo), `client/` (React/Vite), the two frozen
  spec files above, `AGENTS.md`, `.gitignore`.
- **`governance\` (continuity, git-tracked):** `RULEBOOK.md` (master rules —
  PART I–III, PART III = project alignment), `BOOTSTRAP.md`, `ai-context\`
  (+ `archive\`), `work-log\`, `planning\`, `documentation\`.

### Session bootstrap (cheapest first)
1. `AGENTS.md` (this file) → 2. `BOOTSTRAP.md` (always) → 3. latest
   `SESSION-*.md` → 4. latest `LOG-*.md` → 5. `PLAN.md` + `TODO.md` (only when
   today's task touches planning/scope).
- **Continuity location:** all under `governance\` (`ai-context\`, `work-log\`,
  `planning\`, `documentation\`) — tracked in git.

### Rules of the house (Phase 1)
- **Core workflow:** Reusable planning structures → Event Plan → Scheduled
  Event → Execution Copy → Progress Tracking.
- **Server code:** controllers stay short (≈≤20 lines); business rules live in
  services (tree ops, clone, progress, transactions); throw
  `ApiError(statusCode, code, message)`; never raw `Error` for client messages.
- **API envelopes:** success `{ success, data, message }`; failure
  `{ success:false, error:{ code, message, details } }`. HTTP
  `200/201/400/401/403/404/409/500`.
- **Testing standard:** acceptance tests in `server/test/acceptance.test.js`
  using the spec's T-IDs (e.g. `T-09` cycle rejection); TESTING gate = all
  acceptance tests pass + client build clean + the 13-step DoD scenario passes.
  UI-only changes: manual checklist sufficient.
- **Git/env:** never commit unless asked; before any stage/commit/push verify
  `.env` ignore coverage (`git check-ignore -v`); real `.env` never touched.
- **Demo accounts:** seeded local-only demo credentials (Admin/Rahul/Amit at
  `@organishift.dev`, see `server/src/seed/`); NOT recorded here, NOT secrets.

### Quick links
- Specs: root frozen docs plus `governance/planning/` (PLAN, TODO).
- Docs: `governance/documentation/`, session logs `governance/ai-context/`,
  work log `governance/work-log/`.
- Master rules: `governance/RULEBOOK.md` (complete §A–§J + PART I–III).