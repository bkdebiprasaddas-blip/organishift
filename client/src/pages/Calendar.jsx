...
import { utcDateKey, localDateKey, shiftDateKey, formatDateUTC } from '../utils/dates';
...
export default function Calendar() {
  ...
  // Two different keyings are needed here. Month-grid cells are LOCAL midnights
  // built by the browser, so they key on the local day. Event dates are calendar
  // dates persisted as UTC midnight, so they key on the UTC day — reading them
  // with toLocaleDateString() dropped every event into the previous day's cell
  // for users west of Greenwich.
  const dateKey = d => localDateKey(d);
  const eventDayKey = d => utcDateKey(d);

  const eventsByDate = useMemo(() => {
    const map = {};
    events.forEach(e => {
      const startKey = eventDayKey(e.startDate);
      (map[startKey] = map[startKey] || []).push(e);
      // multi-day span — walk calendar dates, inclusive of the end date itself.
      if (e.endDate) {
        const endKey = eventDayKey(e.endDate);
        let cursor = startKey;
        let guard = 0;
        while (cursor < endKey && guard++ < 400) {
          cursor = shiftDateKey(cursor, 1);
          if (!map[cursor]?.includes(e)) (map[cursor] = map[cursor] || []).push(e);
        }
      }
    });
    return map;
      ...
      {dayPopover && (
        <Modal onClose={() => setDayPopover(null)} labelledBy="day-popover-title">
          <h3 id="day-popover-title" className="mb-3 text-base font-bold text-slate-900">
            Events on {formatDateUTC(dayPopover)}
          </h3>
      ...
      {showDialog && (
        <Modal onClose={() => !scheduling && setShowDialog(false)} labelledBy="schedule-title">
          ...
          <div className="space-y-3">
            {plans.length === 0 ? (
              // The old markup rendered a disabled <option value=""> alongside the
              // "Select plan..." <option value="">. Two options with the same
              // value means the informative one can never be the selection, so
              // the user just saw a permanently greyed-out dropdown.
              <p className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-[11px] font-semibold text-amber-800">
                No event plans exist yet. Ask an Admin to create one on the Event Plans page before scheduling.
              </p>
            ) : (
            ...
            <div className="grid grid-cols-2 gap-3">
              <input type="date" value={form.startDate} min={localDateKey(new Date())} onChange={e => setForm(f => ({ ...f, startDate: e.target.value }))} aria-label="Start date" className="rounded-lg border border-slate-300 p-2 text-xs font-semibold" />
