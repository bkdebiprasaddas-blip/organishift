...
> file.
>
> **Owner-approved exception (2026-08-23, per §J0E):** The OrganiShift
> repository adds **PART III — Project-Specific Alignment** at the end of this
> file below, by explicit owner decision. This is a recorded, documented
> exception to the portability note above and applies **only in this repo**.
> PART I and PART II and every §A–§J mandate are unaffected and remain
> universal; this exception allows, but does not require, project specificity.
> In any *other* project this file is placed in, PART III does not apply and
> the portability rule above stands unchanged.
>
...
# PART III — PROJECT-SPECIFIC ALIGNMENT (OrganiShift · Phase 1 MVP)

> **Applicability:** This part is the owner-approved exception recorded in the
> header note (per §J0E / §A.6). It applies **only inside the OrganiShift
> repository**. In any other project, ignore PART III; the portability rule and
> PART I/II govern there.
> **Approved by:** project owner (explicit instruction, 2026-08-23).
> **Relationship:** PART III is **additive** and MUST NOT weaken, remove, or
> bypass any rule in PART I or PART II. Where PART III is silent, PART I/II
> fully apply.

## P1. Project identity
- **Name:** OrganiShift — reusable event planning, scheduling and execution
  tracking system.
- **Phase:** Phase 1 MVP (CRUD-oriented, six pages).
- **Branch:** `v1` (solo project; direct commits to `v1` after user approval).
- **Authoritative frozen specs (root of repo):**
  - `OrganiShift_Phase1_System_Design.docx` — the **System Design** (the
    "what"): scope, architecture, data model, API, UI, roles, algorithms,
    testing, Definition of Done.
  - `OrganiShift_AI_Agent_Execution_Spec.md` — the **AI Agent Execution
    Specification** (the "how"): agent behavior, discipline, verification,
    change control, autonomy, completion criteria.
  - `OrganiShift_Agent_Build_Spec.md` — the **build contract** (task-by-task,
    T-001…T-037): corrections C-1…C-11, source-defect fixes S-1…S-5, derived
    rules. Adds/resolves build detail only; never redefines the frozen source.
  - These three are the frozen baseline; they outrank other project docs for
    scope/architecture/API/UI/roles/testing/DoD. On any conflict, the frozen
    Phase 1 System Design wins unless the user decides otherwise; the build spec
    fills gaps and fixes defects, it does not override the source.

## P2. Source-of-truth hierarchy
1. Explicit user decision in the current task.
2. Frozen Phase 1 System Design.
3. Additive engineering improvements explicitly documented in the System Design.
4. This AI Agent Execution Specification.
5. Existing project code and established conventions.
6. Reasonable engineering judgment.

The agent must not silently override a higher-priority source with a
lower-priority assumption. On conflict: identify it, prefer the frozen Phase 1
scope, make the smallest change, never introduce a product rule merely to ease
implementation, and ask the user only for a genuine product/architectural
decision.

## P3. Phase 1 scope boundary
- **Core workflow:** `Reusable planning structures → Event Plan → Scheduled
  Event → Execution Copy → Progress Tracking`.
- CRUD-oriented MVP, MERN three-tier + modular service layer,
  `React UI → REST API → Route → Controller → Service → Mongoose Model →
  MongoDB`.
- **Explicitly OUT of Phase 1 unless separately authorized:** AI plan
  generation, advanced automation, notifications, reminders, comments,
  attachments, budgets, dependencies, real-time collaboration, Socket.IO,
  weighted progress, marketplace, public template sharing, advanced
  scheduling, complex analytics, unnecessary new collections, unnecessary
  microservices, GraphQL, needless state-management infrastructure.
- No Phase-2 feature may be required for the Phase 1 demo.

## P4. Data model (five collections)
- `users`, `eventPlans`, `planningItems`, `events`, `eventItems`.
- `planningItems` is a self-referencing recursive tree (`parentId`, plus
  `path`, `level`, `order`), with `scope: LIBRARY | PLAN` separating the
  reusable library rows from plan rows; `events`/`eventItems` hold the
  scheduled occurrence and its execution copy.
- **Do NOT** create separate collections for modules, submodules, tasks,
  subtasks, checklists unless the spec changes.
- Enums — Role `ADMIN|MANAGER|MEMBER`; item status
  `NOT_STARTED|IN_PROGRESS|COMPLETED|BLOCKED`; priority
  `LOW|MEDIUM|HIGH|CRITICAL`; event status `PLANNED|ONGOING|DONE`.

## P5. Roles & authorization (backend is the real boundary)
Enforce with role guards + ownership checks on every protected route; the UI
hiding a control is convenience only. Abbreviated matrix:
- **Admin:** manage users, planning library, event plans; add children during
  execution; delete events; re-open completed items.
- **Manager:** (with Admin) schedule events; assign items, set priority/due
  date; add children during execution; re-open completed items.
- **Member:** update status of a leaf item that is assigned to them (owner
  check); see only assigned work where specified.
Never rely on hidden UI controls or frontend state as security.

## P6. API contract
- **Success:** `{ success: true, data: {}, message: "..." }`
- **Failure:** `{ success: false, error: { code, message, details: [] } }`
- **HTTP semantics:** `200` read/update, `201` create, `400` validation /
  business rule, `401` unauthorized, `403` forbidden, `404` not found,
  `409` conflict (e.g. duplicate email), `500` unexpected.
- **Code structure:** Route → short Controller (rule of thumb ≤ ~20 lines) →
  Service (business rules, tree ops, clone, progress, transactions,
  state-dependent validation) → Mongoose Model. Imports flow downward only;
  no circular imports. Do not duplicate business rules in controllers/frontend.
## P7. Security baseline
- bcrypt (10 salt rounds); password hash stripped from every response via the
  schema transform.
- JWT signed with an env secret, 24h expiry, carrying only user id + role;
  Bearer Authorization header, never in a URL.
- `requireRole` middleware on all non-public routes + ownership checks.
- Typed Mongoose queries (no string-built queries); CORS restricted to the
  deployed client origin with credentials disabled; login-rate limiter.
- `.env` holds secrets and is git-ignored; `.env.example` is committed.
- Central error middleware returns a code + safe message; stack traces are
  logged, never returned. Never log passwords, JWT signing secrets, database
  credentials, or private tokens. Never hard-code credentials.

## P8. Core domain invariants (memory of the system)
1. **Tree:** `parentId` + `order` + `path` + `level`; no depth limit; preserve
   sibling ordering.
2. **Cycle protection:** a move must be rejected (`400 CYCLE_DETECTED`) when
   the new parent is inside the moved node's subtree; atomic (no partial tree).
3. **Cascade delete:** deleting a parent deletes its whole subtree atomically;
   no orphans; UI confirms destructive deletion.
4. **Leaf-only execution status:** only leaves are human-editable; parent
   status/percent are derived; a parent cannot become `COMPLETED` while
   descendants are incomplete.
5. **Progress (centralized in the progress service):** `NOT_STARTED=0%`,
   `IN_PROGRESS=50%`, `COMPLETED=100%`, `BLOCKED=0%`. No competing formula in
   the frontend.
6. **Master/execution isolation:** scheduling clones an execution copy; the
   master plan is never mutated; execution items keep traceability via
   `sourcePlanningItemId`/`sourceLibraryItemId`.
7. **Atomic scheduling:** event + full execution copy created together or
   nothing (transaction; a failed clone leaves no half event).

## P9. Working loop & verification discipline
Required loop: `Understand → Inspect → Plan → Implement → Run → Test →
Diagnose → Fix → Retest → Review → Report`. A feature is not complete until its
behavior is verified. Follow §J13/J10: never fake completion; if verification is
impossible, state `NOT VERIFIED` and why. Never suppress errors, mock required
functionality, or claim a pass without executing the test.

## P10. Change control
Classify every change:
- `REQUIRED_FIX` and `BUG_FIX` — proceed autonomously (required to satisfy the
  spec / fix spec-contradicting behavior).
- `ENGINEERING_IMPROVEMENT` — proceed if low risk (no behavior change).
- `SCOPE_CHANGE` — changes product behavior, user workflow, data model, API,
  permissions, Phase-1 features, architecture, or DoD. **Do NOT silently
  implement.** Document the proposal and get approval when not resolvable from
  the spec. (§J12A/J12B mirror this.)

## P11. UI (six pages)
`/login`, `/dashboard`, `/planning-library`, `/event-plans`, `/calendar`,
`/events/:id`. Every page must handle `loading · empty · error · forbidden ·
saving` states; never a blank screen while loading; never silently swallow
errors; prevent duplicate submission; confirm destructive ops. Use one shared
`TreeView` component for library/plan/execution trees driven by `permissions`
and `renderMeta` props. Frontend validation (Zod + React Hook Form) is
convenience; backend validation is authoritative. Keep the six-page structure
unless changed.

## P12. Testing & Definition of Done
Cover: authentication (valid/invalid/protected-route); authorization
(member-vs-admin, ownership); tree (root, deep, order, valid move, cycle
rejection, cascade delete); plan (create, add-from-library, custom item,
master-independence); event (schedule, complete copy, source refs, no-partial);
execution (assign, priority, due, status, leaf-only, progress, overdue); and
UI states. Acceptance tests live in `server/test/acceptance.test.js` and MUST
be reported with the spec's T-IDs (e.g. `T-09 — PASS — moving Food under Rice
returns CYCLE_DETECTED and leaves the tree unchanged`). **Phase 1 is complete
only when the 13-step DoD end-to-end scenario (System Design Appendix B) passes
without manual database edits.**
## P13. Sprint order
1 Scaffold → 2 Authentication → 3 Planning Library → 4 Event Plans →
5 Events (schedule + calendar) → 6 Execution (assign/status/progress) →
7 UI polish (states/responsiveness) → 8 Testing + demo prep.

## P14. Demo seed (local only, NOT production secrets)
Seed via `server/src/seed/`: users **Admin** (ADMIN), **Rahul** (MANAGER),
**Amit** (MEMBER) at `@organishift.dev`; the **Food** and **Stage** reusable
trees; the **Annual Function** plan; the **Annual Function 2026** event. Seed
values live in seed code, not in docs, and are not production secrets.

## P15. Final agent reporting format
After meaningful work, report: **Completed** · **Files Changed** ·
**Verification** (commands/tests + T-IDs + PASS/FAIL) · **Issues** ·
**Specification Impact** (`NONE`, or exact change + classification) ·
**Next Step**. Avoid vague statements ("everything is done", "looks good",
"should work", "all tests pass" without execution).

> End of PART III. For any matter not covered here, PART I and PART II in this
> file and the root specs govern.
