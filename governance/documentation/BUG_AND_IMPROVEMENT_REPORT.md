...
- [x] **SV-L3 — Insecure JWT fallback secret when NODE_ENV unset.** `config/env.js`. Fix: fail-closed unless explicitly development. Verify: run with no JWT_SECRET + no NODE_ENV → server refuses to start. — **NOT actually fixed until 2026-09-26.** The original "fix" tested `NODE_ENV !== 'development'` while `NODE_ENV` itself *defaults* to `development`, so the exact scenario in the verify step (no `JWT_SECRET`, no `NODE_ENV`) took the fallback branch and signed tokens with the public literal `dev-only-insecure-secret-change-me`. Now fails closed unconditionally, requires >= 16 chars, rejects documented placeholders in production, and gates the dev fallback behind an explicit `ALLOW_INSECURE_DEV_SECRET=true`. Verified against the verify-step scenario directly.
